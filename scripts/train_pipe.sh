...
get_latest_checkpoint() {
    local save_dir=$1
    local latest_version_dir=$(ls -d ${save_dir}/version_* 2>/dev/null | sort -V | tail -n 1)
    if [ -n "$latest_version_dir" ]; then
        local latest_ckpt=`cat ${latest_version_dir}/checkpoint/topk_map.txt | head -n 1 | awk -F " " '{print $2}'`
        echo $latest_ckpt
    fi
}
...
# AE_CKPT="ckpts/unimomo/AE_unimomo/version_7/checkpoint/epoch19_step62980.ckpt"
AE_CKPT=$(get_latest_checkpoint $AE_SAVE_DIR)
